...
def set_exp(request):
    if request.method == 'POST':
        ...
        inputValue = new_data.get("inputValue")
        qinmidu = new_data.get("qinmidu")
        if ExpResult.objects.filter(exp_id=exp_id, index=index) is not None:
            ...
            ExpResult.objects.update_or_create(exp_id=exp_id,index=index,self_val=self_val,computer_val=computer_val,reaction_time=reaction_time,time=time,option=option,other_option=other_option,inputValue = inputValue,qinmidu = qinmidu)
